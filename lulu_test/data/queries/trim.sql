...
INSERT INTO EpcMax (id, productId, storeId, yyyy, mm, dd, h, m, isMove, isRegion, isExit, isMissing, isReacquired, isValid, isGhost, minX, minY, maxX, maxY, avgX, avgY, ts, total, intervalTotal, tempKey) SELECT id, productId, storeId, yyyy, mm, dd, HOUR(ts), FLOOR(MINUTE(ts)/5), MAX(isMove), MAX(isRegion), MAX(isExit), MAX(isMissing), MAX(isReacquired), MAX(isValid), MAX(isGhost), MIN(x), MIN(y), MAX(x), MAX(y), AVG(x), AVG(y), MAX(ts), COUNT(*), 0, CONCAT(id, '_', storeId, '_', CAST(yyyy AS CHAR), '_', CAST(mm AS CHAR), '_', CAST(dd AS CHAR), '_', CAST(HOUR(ts) AS CHAR), '_', CAST(FLOOR(MINUTE(ts)/5) AS CHAR)) FROM EpcMovement GROUP BY id, storeId, productId, yyyy, mm, dd, HOUR(ts), FLOOR(MINUTE(ts)/5);
...
UPDATE EpcMovement SET tempKey = CONCAT(id, '_', storeId, '_', CAST(yyyy AS CHAR), '_', CAST(mm AS CHAR), '_', CAST(dd AS CHAR), '_', CAST(HOUR(ts) AS CHAR), '_', CAST(FLOOR(MINUTE(ts)/5) AS CHAR));
...
RENAME EpcMovement TO EpcMovement_BACKUP;
