...
CREATE VIEW CurrentLocation AS SELECT m.storeId, m.id, m.productId, p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, p.classCOde, p.className, p.subClassCode, p.subClassName, p.styleCode, p.styleName, m.x, m.y, m.z, m.confidence, m.ts, m.regionId, m.regionName, m.yyyy, m.mm, m.dd FROM EpcMovement m JOIN Products p ON m.productId=p.sku JOIN ValidEpc cl ON cl.idx=m.idx;

DROP VIEW IF EXISTS EpcMoveView;
CREATE VIEW EpcMoveView AS SELECT m.storeId, m.id, m.productId, p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, p.classCOde, p.className, p.subClassCode, p.subClassName, p.styleCode, p.styleName, m.x, m.y, m.z, m.confidence, m.ts, m.regionId, m.regionName, m.yyyy, m.mm, m.dd FROM EpcMovement m JOIN Products p ON m.productId=p.sku ; 
