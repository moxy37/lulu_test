...
CREATE VIEW AllDept AS SELECT p.deptCode, p.deptName, COUNT(*) AS total, x.storeId  FROM (SELECT storeId, id, productId FROM EpcMovement GROUP BY storeId, productId, id)  AS x JOIN Products p ON x.productId=p.sku GROUP BY x.storeId, p.deptCode, p.deptName;

DROP VIEW IF EXISTS AllSubDept;
CREATE VIEW AllSubDept AS SELECT p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, COUNT(*) AS total, x.storeId  FROM (SELECT storeId, id, productId FROM EpcMovement GROUP BY storeId, productId, id)  AS x JOIN Products p ON x.productId=p.sku GROUP BY x.storeId, p.deptCode, p.deptName, p.subDeptCode, p.subDeptName;

DROP VIEW IF EXISTS AllClass;
CREATE VIEW AllClass AS SELECT p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, p.classCode, p.className, COUNT(*) AS total, x.storeId  FROM (SELECT storeId, id, productId FROM EpcMovement GROUP BY storeId, productId, id)  AS x JOIN Products p ON x.productId=p.sku GROUP BY x.storeId, p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, p.classCode, p.className;
...
CREATE VIEW EpcMovementView AS SELECT m.storeId, m.regionId, m.id, m.idx, m.productId, p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, p.classCode, p.className, p.subClassCode, p.subClassName, p.styleCode, p.styleName, p.price, m.x, m.y, m.z, m.confidence, m.ts, m.yyyy, m.mm, m.dd, m.isDeleted, m.isDeparture, m.isExit, m.isGhost, m.isMissing, m.isMove, m.isReacquired, m.isRegion, m.isSold, m.isValid, r.regionName, s.storeName FROM EpcMovement m JOIN Products p ON m.productId=p.sku JOIN Regions r ON m.regionId=r.id JOIN Stores s ON m.storeId=s.id;
