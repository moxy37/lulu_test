...
CREATE VIEW AllStyleGroup AS SELECT storeId, deptCode, deptName, subDeptCode, subDeptName, classCode, className, subClassCode, subClassName, SUBSTRING(styleName, 1, 12) AS styleGroup, SUM(total) AS total FROM AllStyle GROUP BY storeId, deptCode, deptName, subDeptCode, subDeptName, classCode, className, subClassCode, subClassName, SUBSTRING(styleName, 1, 15);

DROP VIEW IF EXISTS CurrentLocation;
CREATE VIEW CurrentLocation AS SELECT m.storeId, m.id, m.productId, p.deptCode, p.deptName, p.subDeptCode, p.subDeptName, p.classCOde, p.className, p.subClassCode, p.subClassName, p.styleCode, p.styleName, SUBSTRING(p.styleName, 1, 15) AS styleGroup, m.x, m.y, m.z, m.confidence, m.ts, m.regionId, m.regionName, m.yyyy, m.mm, m.dd, m.isDeparture, m.isExit, m.isGhost, m.isMissing, m.isMove, m.isReacquired, m.isRegion, m.isSold, m.isValid FROM EpcMovement m JOIN Products p ON m.productId=p.sku JOIN ValidEpc cl ON cl.id=m.id AND m.ts=cl.ts;
